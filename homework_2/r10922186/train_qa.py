...
fields = ["encoder", "bs", "optimizer", "lr"]
if args.nopt: # no pre-trained
    fields.append("nopt")
args.exp_name = render_exp_name(args, fields=fields)
...
if args.nopt: # if no pre-trained
    if args.nopt == "small":
        # Set config
        small_config = {"hidden_size": 512, "num_attention_heads": 8, "num_hidden_layers": 4, "intermediate_size": 2048}
        for k, v in small_config.items():
            setattr(config, k, v)
    elif args.nopt == "mini":
        mini_config = {"hidden_size": 256, "num_attention_heads": 4, "num_hidden_layers": 4, "intermediate_size": 1024}
        for k, v in mini_config.items():
            setattr(config, k, v)
